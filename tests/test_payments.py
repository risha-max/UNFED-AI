...
class TestPaymentContract:
    ...
    def test_infra_split_pays_nodes_first_registry_gets_remainder(self, payment_contract):
        payment_contract.deposit_to_escrow(1000.0)
        payment_contract.set_infra_policy(
            daemon_recipient="daemon_1",
            daemon_fee_bps=300,   # 3% registry pool
        )
        ...
        # Compute nodes receive full settlement first.
        assert split["node_A"] == pytest.approx(0.036, rel=1e-9)
        assert split["node_B"] == pytest.approx(0.024, rel=1e-9)
        # Registry gets only any residual remainder (typically zero).
        assert split.get("daemon_1", 0.0) == pytest.approx(0.0, abs=1e-12)
