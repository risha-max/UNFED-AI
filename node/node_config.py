...
class ComputeConfig(BaseNodeConfig):
    ...
    he_compute_mode: str = "off"                 # off|decode_client_sample|full_output_2pc
...
def _validate(cfg: NodeConfig):
    ...
    if isinstance(cfg, MPCConfig):
        ...
        if cfg.he_compute_mode not in ("off", "decode_client_sample", "full_output_2pc"):
            raise ValueError(
                "he_compute_mode must be off|decode_client_sample|full_output_2pc"
            )
