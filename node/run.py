...
Examples:
  ...
  python -m node.run --role compute --shard-index 0 --port 50051
    ...
    parser.add_argument("--he-compute-mode", type=str, default=None,
                        choices=["off", "decode_client_sample", "full_output_2pc"],
...
def main():
    ...
    if isinstance(cfg, MPCConfig):
        ...
        serve_daemon(
            ...
            registry_address=cfg.registry,
            db_path=cfg.db_path,
            chain_prune_keep=cfg.chain_prune_keep,
            block_interval_seconds=cfg.block_interval_seconds,
            settlement_blocks=cfg.settlement_blocks,
            collection_window_seconds=cfg.collection_window_seconds,
            peer_refresh_interval_seconds=cfg.peer_refresh_interval_seconds,
            gossip_timeout_seconds=cfg.gossip_timeout_seconds,
            sync_timeout_seconds=cfg.sync_timeout_seconds,
            fee_enabled=cfg.fee_enabled,
            fee_base=cfg.fee_base,
            fee_min=cfg.fee_min,
            fee_max=cfg.fee_max,
            fee_adjustment_factor=cfg.fee_adjustment_factor,
            fee_target_utilization=cfg.fee_target_utilization,
            fee_window_blocks=cfg.fee_window_blocks,
            fee_target_capacity=cfg.fee_target_capacity,
            heartbeat_interval_seconds=cfg.heartbeat_interval_seconds,
            grpc_max_workers=cfg.grpc_max_workers,
            grpc_options=cfg.grpc_options,
        )
