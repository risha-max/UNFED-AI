...
class RacingCoordinator:
    ...
    def _compute_response_hash(self, response: inference_pb2.ForwardResponse) -> str:
        ...
        if response.output_2pc_artifact_hash:
            h.update(str(response.output_2pc_artifact_hash).encode("utf-8"))
        if response.output_2pc_stage:
            h.update(str(response.output_2pc_stage).encode("utf-8"))
        if response.output_2pc_artifact_type:
